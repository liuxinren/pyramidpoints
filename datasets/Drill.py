        ...
        for i, file_path in enumerate(files):
            ...
            if isfile(KDTree_file):
                ...
                if cloud_split == 'test':
                    ...
                    sub_labels = data['class'].astype(np.int32)
                ...
                reflectance = np.zeros([data['x'].shape[0],1]).astype(np.float32)#np.expand_dims(data['scalar_reflectance'], 1).astype(np.float32)

                if cloud_split == 'test':
                    int_features = None
                else:
                    int_features = data['scalar_Classification'].astype(np.int32)
