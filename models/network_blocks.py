...
def pyramid_upsample_2(layer_ind, inputs, features, radius, fdim, config, training):
    fdim = fdim // 2


    features1 = nearest_upsample_block(layer_ind, inputs, features, radius, fdim, config, training)
    features1 = unary_block(layer_ind, inputs, features1, radius, fdim, config, training)

    fdim = fdim // 2

    features2 = nearest_upsample_block(layer_ind-1, inputs, features1, radius, fdim, config, training)
    features2 = unary_block(layer_ind-1, inputs, features2, radius, fdim, config, training)

    return features1, features2

def pyramid_upsample_3(layer_ind, inputs, features, radius, fdim, config, training):
    fdim = fdim // 2
    ...
    features2 = nearest_upsample_block(layer_ind-1, inputs, features1, radius, fdim, config, training)
    features2 = unary_block(layer_ind, inputs, features2, radius, fdim, config, training)

    fdim = fdim // 2

    features3 = nearest_upsample_block(layer_ind-2, inputs, features2, radius, fdim, config, training)
    features3 = unary_block(layer_ind, inputs, features3, radius, fdim, config, training)
...
def get_block_ops(block_name):
    ...
    elif block_name in 'nearest_upsample':
    ...
    elif block_name =='pyramid2':
        return pyramid_upsample_2

    elif block_name == 'pyramid3':
        return pyramid_upsample_3
...
def assemble_CNN_blocks(inputs, config, dropout_prob):
    ...
    for block_i, block in enumerate(config.architecture):
        ...
        with tf.variable_scope('layer_{:d}/{:s}_{:d}'.format(layer, block, block_in_layer)):
            ...
            if layer == 2 and block_in_layer == 1:
                print('pyramid')
                block_ops = get_block_ops("pyramid2")
                f_3_2, f_3_2_1, = block_ops(layer,
                                            inputs,
                                            features,
                                            r,
                                            fdim,
            ...
            if layer == 3 and block_in_layer == 1:
                print('pyramid')
                block_ops = get_block_ops("pyramid3")
                f_4_3, f_4_3_2, f_4_3_2_1 = block_ops(layer,
                                            inputs,
                                            features,
                                            r,
                                            fdim,
    ...
    return F, f_4_3, f_3_2 + f_4_3_2,  f_3_2_1+f_4_3_2_1
...
def assemble_FCNN_blocks(inputs, config, dropout_prob):
    ...
    F, f_layer3, f_layer2, f_layer1 = assemble_CNN_blocks(inputs, config, dropout_prob)
    features = F[-1]
    ...
    for block_i, block in enumerate(config.architecture[start_i:]):
        ...
        with tf.variable_scope('uplayer_{:d}/{:s}_{:d}'.format(layer, block, block_in_layer)):
            ...
            if "unary" in block:
                if layer == 2:
                    features = features + f_layer3
                if layer == 1:
                    features = features + f_layer2
                if layer == 0:
                    features = features + f_layer1
